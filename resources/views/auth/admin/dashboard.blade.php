...
@section('content')
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-xl-2 col-md-4 col-6">
            <div class="stat-card">
                <div class="stat-icon"
                    style="background: linear-gradient(135deg, rgba(59, 130, 246, 0.2), rgba(37, 99, 235, 0.2)); color: #60a5fa;">
                ...
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <div class="btn-group btn-group-sm">
                        <button type="button" class="btn btn-outline-secondary active"
                            onclick="switchChart('daily')">Harian</button>
                        <button type="button" class="btn btn-outline-secondary"
                            onclick="switchChart('monthly')">Bulanan</button>
                    </div>
                </div>
                <div class="card-body">
                                            ...
                                            <div class="d-flex align-items-center">
                                                @if ($loan->book && $loan->book->primaryImage)
                                                    <img src="{{ $loan->book->primaryImage->image_url }}"
                                                        class="rounded me-2" width="30" height="40"
                                                        style="object-fit: cover;"
                                                        onerror="this.src='https://via.placeholder.com/30x40?text=B'">
                                                @else
                                                    <div class="bg-secondary rounded me-2 d-flex align-items-center"
                    ...
                    @forelse($topBooks as $index => $book)
                        <div class="d-flex align-items-center p-3 border-bottom {{ $loop->last ? 'border-0' : '' }}">
                            ...
                            <div class="flex-grow-1">
                                ...
                                <small class="text-success fw-medium">{{ number_format($book->loan_count ?? 0) }}x
                                    dipinjam</small>
                            </div>
                            <div class="text-end">
                    ...
                    @forelse($overdueMembers as $index => $loan)
                        <div class="d-flex align-items-center p-3 border-bottom {{ $loop->last ? 'border-0' : '' }}">
                            ...
                            <div class="flex-grow-1">
                                ...
                                <small
                                    class="text-danger fw-medium">{{ Str::limit($loan->book->name ?? 'Buku', 20) }}</small>
                            </div>
                            <div class="text-end">
                    ...
                    <div class="timeline">
                        @forelse($recentActivities as $activity)
                            <div class="timeline-item">
                                <div
                                    class="timeline-marker {{ $activity['type'] === 'return' ? 'bg-success' : 'bg-primary' }}">
                                </div>
                                ...
                                <div class="timeline-end">
                                    <span
                                        class="badge bg-{{ $activity['status'] === 'overdue' ? 'danger' : ($activity['status'] === 'returned' ? 'success' : 'primary') }}">
