                                        ...
                                        <div class="form-check mb-2 ps-0 genre-item">
                                            <label class="form-check-label d-flex align-items-center cursor-pointer"
                                                ...
                                                <span class="badge bg-secondary ms-auto"
                                                    style="font-size: 0.7rem;">{{ $products->total() }}</span>
                                            </label>
                                        </div>
                                        ...
                                        @foreach ($genreList as $genre)
                                            <div class="form-check mb-2 ps-0 genre-item"
                                                ...
                                                <label class="form-check-label d-flex align-items-center cursor-pointer"
                                                    ...
                                                    <span class="text-light">{{ $genre['icon'] }}
                                                        {{ $genre['name'] }}</span>
                                                    <span class="badge bg-secondary ms-auto"
                                                        style="font-size: 0.7rem;">{{ $genre['count'] }}</span>
                                                </label>
                                            </div>
                                        @endforeach
                                    </div>
                    ...
                    <div class="d-lg-none mb-3">
                        ...
                        <div class="collapse mt-2" id="mobileFilterCollapse">
                            <div class="card border-0"
                                style="background: rgba(30, 41, 59, 0.95); border: 1px solid #334155 !important;">
                                <div class="card-body">
                                        ...
                                        <div class="filter-group mb-3">
                                            ...
                                            <div class="d-flex flex-wrap gap-2">
                                                <input type="radio" class="btn-check" name="category"
                                                    id="mobile_category_all" value=""
                                                    {{ !request('category') ? 'checked' : '' }}>
                                                <label class="btn btn-sm btn-outline-secondary"
                                                    for="mobile_category_all">Semua</label>

                                                @foreach ($genreList as $genre)
                                                    <input type="radio" class="btn-check" name="category"
                                                        id="mobile_category_{{ $genre['slug'] }}"
                                                        value="{{ $genre['slug'] }}"
                                                        {{ request('category') == $genre['slug'] ? 'checked' : '' }}>
                                                    <label class="btn btn-sm btn-outline-secondary"
                                                        for="mobile_category_{{ $genre['slug'] }}">{{ $genre['icon'] }}
                                                        {{ $genre['name'] }}</label>
                                                @endforeach
                                            </div>
                                        </div>
                            ...
                            <div class="col-lg-4">
                                ...
                                <div class="search-wrapper position-relative">
                                    <div class="input-group">
                                        ...
                                        <input type="text" name="q" id="live-search-input" class="form-control"
                                            placeholder="Cari buku..." value="{{ request('q', '') }}"
                                            autocomplete="off">
                                    </div>
                                    {{-- Autocomplete Dropdown --}}
                                    <div id="search-suggestions" class="search-suggestions position-absolute w-100 mt-1">
                                    </div>
                                </div>
                        ...
                        <div class="active-filters mb-3 d-flex flex-wrap gap-2 align-items-center">
                            ...
                            @if (request('category'))
                                ...
                                <span class="badge bg-primary d-flex align-items-center">
                                    {{ $genreName }}
                                    <a href="{{ route('catalog.index', array_diff_key(request()->all(), ['category' => ''])) }}"
                                        class="text-white ms-1 text-decoration-none">&times;</a>
                                </span>
                            @endif
                            @if (request('min_price') || request('max_price'))
                                <span class="badge bg-info text-dark d-flex align-items-center">
                                    Rp {{ number_format(request('min_price', 0), 0, ',', '.') }} - Rp
                                    {{ number_format(request('max_price', 0), 0, ',', '.') }}
                                    <a href="{{ route('catalog.index', array_diff_key(request()->all(), ['min_price' => '', 'max_price' => ''])) }}"
                                        class="text-dark ms-1 text-decoration-none">&times;</a>
                                </span>
                            @endif
                            @if (request('q'))
                                <span class="badge bg-warning text-dark d-flex align-items-center">
                                    "{{ request('q') }}"
                                    <a href="{{ route('catalog.index', array_diff_key(request()->all(), ['q' => ''])) }}"
                                        class="text-dark ms-1 text-decoration-none">&times;</a>
                                </span>
                            @endif
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function renderSuggestions(products) {
                let html =
                    '<div class="search-suggestions-list shadow" style="background: rgba(30, 41, 59, 0.98); border: 1px solid #334155; border-radius: 0.5rem;">';
