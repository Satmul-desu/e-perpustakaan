    ...
    <div class="detail-header-card">
        <div class="d-flex justify-content-between align-items-start flex-wrap gap-3">
            ...
            <div class="d-flex gap-2 flex-wrap">
                <span class="status-badge bg-{{ $complaint->status === 'pending' ? 'warning' : ($complaint->status === 'resolved' ? 'success' : 'info') }}">
                    <i class="bi bi-{{ $complaint->status === 'pending' ? 'clock' : ($complaint->status === 'resolved' ? 'check-circle' : 'arrow-repeat') }} me-1"></i>
                    {{ $complaint->status_name }}
                </span>
                <span class="priority-badge bg-{{ $complaint->priority === 'urgent' ? 'danger' : ($complaint->priority === 'high' ? 'warning' : 'secondary') }}">
                        ...
                        <div class="message-bubble admin">
                            <div class="message-meta">
                                ...
                                <div>
                                    ...
                                    <span class="message-time">• {{ $complaint->responded_at?->format('d/m/Y H:i') }}</span>
                                </div>
                            </div>
                ...
                <div class="info-body">
                    ...
                    @if($complaint->order_number)
                        <div class="info-item">
                            ...
                            <div class="info-value">
                                <code style="background: rgba(129, 140, 248, 0.1); padding: 0.25rem 0.5rem; border-radius: 6px; color: #818cf8;">
                    ...
                    <div class="status-timeline">
                        ...
                        <div class="timeline-item">
                            <div class="timeline-dot {{ in_array($complaint->status, ['processing', 'resolved']) ? 'completed' : ($complaint->status === 'pending' ? 'active' : '') }}"></div>
                            <span class="timeline-text {{ $complaint->status !== 'pending' ? 'active' : '' }}">Diproses</span>
                        </div>
                        <div class="timeline-item">
                            <div class="timeline-dot {{ $complaint->status === 'resolved' ? 'completed' : '' }}"></div>
                            <span class="timeline-text {{ $complaint->status === 'resolved' ? 'active' : '' }}">Selesai</span>
                        </div>
                    </div>
