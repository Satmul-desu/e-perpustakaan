            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 gap-2">
                ...
                <div class="d-flex align-items-center gap-2">
                    ...
                    <select class="form-select form-select-sm"
                        style="width: auto; background: rgba(30, 41, 59, 0.8); color: white; border: 1px solid #334155;"
                        onchange="window.location.href = this.value">
                        <option value="{{ request()->fullUrlWithQuery(['sort' => 'newest']) }}"
                            {{ !request('sort') || request('sort') == 'newest' ? 'selected' : '' }}>Terbaru</option>
                        <option value="{{ request()->fullUrlWithQuery(['sort' => 'price-low']) }}"
                            {{ request('sort') == 'price-low' ? 'selected' : '' }}>Harga: Rendah ke Tinggi</option>
                        <option value="{{ request()->fullUrlWithQuery(['sort' => 'price-high']) }}"
                            {{ request('sort') == 'price-high' ? 'selected' : '' }}>Harga: Tinggi ke Rendah</option>
                            ...
                            @if ($flashSaleProducts->onFirstPage())
                                <li class="page-item disabled">
                                    <span class="page-link"
                                        style="background: rgba(30, 41, 59, 0.8); border: 1px solid #334155; color: white;">
                                        <i class="bi bi-chevron-left"></i>
                                    </span>
                                </li>
                            @else
                                <li class="page-item">
                                    <a class="page-link" href="{{ $flashSaleProducts->previousPageUrl() }}"
                                        style="background: rgba(30, 41, 59, 0.8); border: 1px solid #334155; color: white;">
                                        <i class="bi bi-chevron-left"></i>
                                    </a>
                                </li>
                            ...
                            @foreach ($flashSaleProducts->links()->elements[0] as $page => $url)
                                @if ($page == $flashSaleProducts->currentPage())
                                    <li class="page-item active">
                                        <span class="page-link"
                                            style="background: #dc2626; border-color: #dc2626;">{{ $page }}</span>
                                    </li>
                                @else
                                    <li class="page-item">
                                        <a class="page-link" href="{{ $url }}"
                                            style="background: rgba(30, 41, 59, 0.8); border: 1px solid #334155; color: white;">{{ $page }}</a>
                                    </li>
                                @endif
                            @endforeach
                            @if ($flashSaleProducts->hasMorePages())
                                <li class="page-item">
                                    <a class="page-link" href="{{ $flashSaleProducts->nextPageUrl() }}"
                                        style="background: rgba(30, 41, 59, 0.8); border: 1px solid #334155; color: white;">
            ...
            <div class="cta-card">
                <div class="row align-items-center">
                    <div class="col-lg-8 mb-3 mb-lg-0">
                        ...
                        <p class="text-secondary mb-0">Jelajahi katalog lengkap kami untuk menemukan lebih banyak buku
                            menarik.</p>
                    </div>
